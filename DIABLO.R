...
set.seed(123)
metab <- read.csv(file.path(input_dir, "met_data v2.csv"), check.names = FALSE, row.names = 1)
lipid <- read.csv(file.path(input_dir, "lip_data v2.csv"), check.names = FALSE, row.names = 1)
meta <- read.csv(file.path(input_dir, "sample_info.csv"), check.names = FALSE, row.names = 1)
...
metab_keep <- names(sort(metab_cv, decreasing = TRUE)[1:floor(0.95 * length(metab_cv))])
lipid_keep <- names(sort(lipid_cv, decreasing = TRUE)[1:floor(0.95 * length(lipid_cv))])
...
basic.model <- block.splsda(X = blocks, Y = Y, ncomp = max_ncomp, design = design)

set.seed(123)
perf.diablo <- perf(basic.model, validation = 'Mfold', folds = 5, nrepeat = 10,
...
tune.res <- tune.block.splsda(
  X = blocks, Y = Y, ncomp = ncomp_optimal, test.keepX = test.keepX,
  design = design, validation = 'Mfold', folds = 5, nrepeat = 50,
...
adjusted.ncomp <- min(ncomp_optimal, length(optimal.keepX$METAB), length(optimal.keepX$LIPID))
...
set.seed(123)
temp.model <- block.splsda(X = blocks, Y = Y, ncomp = adjusted.ncomp,
                           keepX = keepX_adjusted, design = design)
...
set.seed(123)
perf.res <- perf(temp.model, validation = "Mfold", folds = 5, nrepeat = 50,
                 dist = "centroids.dist", progressBar = FALSE)

stab_df <- data.frame(
  Feature = names(perf.res$features$stable$METAB),
  Stability = as.numeric(perf.res$features$stable$METAB)
)
stab_df <- stab_df[order(stab_df$Stability, decreasing = TRUE), ]
write.csv(stab_df, file.path(output_dir, "feature_stability_METAB.csv"), row.names = FALSE)
...
set.seed(123)
diablo.fit <- block.splsda(X = blocks, Y = Y, ncomp = adjusted.ncomp,
                           keepX = keepX_adjusted, design = design)
...
if (!is.null(diablo.fit)) {
  ...
  circosPlot(diablo.fit, cutoff = 0.3, legend = TRUE, line = TRUE,
             size.variables = 0.5, color.blocks = c(METAB = "#E41A1C", LIPID = "#377EB8"),
             track.height = 0.1, show.block.names = TRUE)
  dev.off()
}
...
plot_loadings <- function(fit, block, comp, filename) {
  ...
  if (comp > model_ncomp) return(NULL)
...
cimDiablo(diablo.fit, comp = 1, title = "DIABLO Heatmap - Component 1", legend.position = "right")
dev.off()

if(adjusted.ncomp >= 2) {
  pdf(file.path(output_dir, "heatmap_cimDiablo_com2.pdf"), width = 12, height = 10)
  cimDiablo(diablo.fit, comp = 2, title = "DIABLO Heatmap - Component 2", legend.position = "right")
  dev.off()
}
